...
class BaseEvaluation:
    ...
    def _get_chunks_and_metadata(self, splitter):
        ...
        documents = []
        metadatas = []
        for corpus_id in self.questions_df['corpus_id'].unique():
            
            # 修正這一段：在進行查詢前，先將 corpus_id 標準化
            standardized_corpus_id = str(corpus_id).replace('\\', '/')
    ...
    def _chunker_to_collection(self, chunker, embedding_function, chroma_db_path:str = None, collection_name:str = None):
        ...
        if collection is None:
            ...
            try:
                # 嘗試刪除集合，如果不存在則會拋出 NotFoundError
                self.chroma_client.delete_collection(collection_name)
                print(f"Existing collection '{collection_name}' deleted.")
            except chromadb.errors.NotFoundError:
                # 如果集合不存在，捕捉這個錯誤並忽略，這正是我們想要的行為
    ...
    def run(self, chunker, embedding_function: Callable = None, retrieve: int = 5, db_to_save_chunks: str = None):
        ...
        if db_to_save_chunks is not None:
            ...
            # The embedding function is now always BGE-M3
        ...
        if self.is_general:
            with resources.as_file(resources.files('chunking_evaluation.evaluation_framework') / 'general_evaluation_data') as general_benchmark_path:
                ...
                try:
                    question_collection = questions_client.get_collection("auto_questions_bge_m3", embedding_function=embedding_function)
                except Exception as e:
        ...
        if not self.is_general or question_collection is None:
            # Use a more specific try/except to handle the deletion gracefully.
            try:
                self.chroma_client.delete_collection("auto_questions")
                print("Existing 'auto_questions' collection deleted.")
            except Exception as e:
                # A more generic exception catch to handle both ValueError and NotFoundError
                print(f"Collection 'auto_questions' did not exist or could not be deleted: {e}")
                pass

            # Ensure the collection is created successfully

            if self.questions_df.empty:
                print("Fatal Error: `questions_df` is empty. No queries to add to the collection.")
                # 你可以選擇 raise ValueError("Questions DataFrame is empty.")
                return # 或者直接返回，防止程式崩潰
            ...
            try:
                ...
                print(f"Error creating or populating 'auto_questions' collection: {e}")
                # Re-raise the exception or handle it to prevent the program from continuing with an invalid collection
                raise e
